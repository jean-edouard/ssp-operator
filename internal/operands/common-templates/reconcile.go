...
func (c *commonTemplates) PauseCRs(request *common.Request) error {
	...
	err := request.Client.List(request.Context, &kubevirtCommonTemplatesBundles, &client.ListOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
			request.Logger.Info(fmt.Sprintf("No legacy common template bundle CR found"))
			return nil
		} else {
			request.Logger.Error(err, fmt.Sprintf("Error listing common template bundle CRs: %s", err))
			return err
		}
	}
	for _, kubevirtCommonTemplatesBundle := range kubevirtCommonTemplatesBundles.Items {
		err = request.Client.Patch(request.Context, &kubevirtCommonTemplatesBundle,
